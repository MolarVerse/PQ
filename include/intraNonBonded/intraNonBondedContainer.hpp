...
class intraNonBonded::IntraNonBondedContainer
{
  ...
  public:
    IntraNonBondedContainer(const size_t molType, const std::vector<std::vector<size_t>> &atomIndices)
        : _molType(molType), _atomIndices(atomIndices)
    {
    }

    [[nodiscard]] size_t                           getMolType() const { return _molType; }
    [[nodiscard]] std::vector<std::vector<size_t>> getAtomIndices() const { return _atomIndices; }
};
