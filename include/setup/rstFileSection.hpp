...
namespace Setup::RstFileReader
{
    ...
    {
    private:
        void checkAtomLine(std::vector<std::string> &, std::string &, const std::unique_ptr<Molecule> &);
        void processAtomLine(std::vector<std::string> &, const std::unique_ptr<Molecule> &) const;
