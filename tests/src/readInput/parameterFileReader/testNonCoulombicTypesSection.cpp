...
//     EXPECT_EQ(_engine->getForceField().getNonCoulombicPairsVector().size(), 1);
//     auto *pair = dynamic_cast<const forceField::BuckinghamPair
//     *>(_engine->getForceField().getNonCoulombicPairsVector()[0].get()); EXPECT_EQ(pair->getVanDerWaalsType1(), 0);
//     EXPECT_EQ(pair->getVanDerWaalsType2(), 1);
...
//     EXPECT_NO_THROW(dynamic_cast<const forceField::MorsePair
//     *>(_engine->getForceField().getNonCoulombicPairsVector()[0].get()));
