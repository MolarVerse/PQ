...
#include "defaults.hpp"            // for manostat defaults
...
class TestInputValidation : public ::testing::Test
{
   protected:
    void SetUp() override
    {
        ...
        ThermostatSettings::setRelaxationTime(0.1);
        ...
        PotentialSettings::setCoulombRadiusCutOff(12.5);
...
TEST_F(TestInputValidation, acceptsEndTemperatureForThermostat)
{
    ...
    EXPECT_NO_THROW(_reader->validateInputConfiguration());
    EXPECT_DOUBLE_EQ(ThermostatSettings::getTargetTemperature(), 300.0);
    EXPECT_DOUBLE_EQ(ThermostatSettings::getActualTargetTemperature(), 300.0);
}

TEST_F(TestInputValidation, usesStartTemperatureForVelocityInitialization)
{
    configureMDJob(JobType::MM_MD);
    ThermostatSettings::setThermostatType(ThermostatType::BERENDSEN);
    ThermostatSettings::setTargetTemperature(300.0);
    ThermostatSettings::setStartTemperature(200.0);
    setKeyword("temp");
    setKeyword("start_temp");

    EXPECT_NO_THROW(_reader->validateInputConfiguration());
    EXPECT_DOUBLE_EQ(ThermostatSettings::getActualTargetTemperature(), 200.0);
}
