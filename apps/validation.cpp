...
namespace
{
    ...
    )
    {
        if (!std::filesystem::is_regular_file(fileName))
            throw customException::InputFileException(
                std::format("{} \"{}\" does not exist", description, fileName)
            );
    }
    ...
    void validateInstalledExternalQMScript()
    {
        ...
        {
            requireFile(QM::bundledQMScriptPath(script), "Bundled QM script");

            const auto scripts =
                cli::externalQMScripts(QMSettings::getQMMethod());
            const auto selected = std::ranges::find_if(
                scripts,
                [&script](const auto &candidate)
                { return candidate.name == script; }
            );
            if (selected != scripts.end() &&
                !selected->requiredWorkingFile.empty())
                requireFile(
    ...
    void validateEffectiveFiles(engine::Engine &engine)
    {
        ...
        if (Settings::isQMActivated() &&
            settings::QMSettings::getQMMethod() == settings::QMMethod::DFTBPLUS)
        ...
        if (Settings::isQMActivated() &&
            settings::QMSettings::getQMMethod() == settings::QMMethod::FENNOL)
            requireFile(
...
/**
 * @brief Parses and checks an input file without starting a simulation.
 *
 * @param inputFile
...
/**
 * @brief Writes a machine-readable validation result.
 *
 * @param result
 * @param output
 */
...
/**
 * @brief Writes a concise validation result for interactive use.
 *
 * @param result
 * @param output
 * @param error
