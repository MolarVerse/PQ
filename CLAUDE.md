# CLAUDE.md — AI Assistant Guide for PQ

This file documents the codebase structure, development workflows, and conventions that AI assistants must follow when contributing to PQ.
...
5. AI-generated branches should follow the pattern: `claude/<short-description>-<session-id>`.
6. **Always `git checkout dev` and `git pull` to update local `dev` before creating any new branch.**
7. **Never amend or force-push** to shared branches.
8. Use `git push -u origin <branch-name>` when pushing a new branch.
9. **Install the commit-msg hook once per clone**: `cp .githooks/commit-msg .git/hooks/`. It rejects commits whose subject doesn't start with a recognized conventional-commit-style prefix (see below) — this is enforced locally, not just in CI.
...
Example: `docs: add CLAUDE.md AI assistant guide`
...
- [ ] Branch created from `dev` (not `main`), named `feature/...`, `bugfix/...`, or `claude/...`
